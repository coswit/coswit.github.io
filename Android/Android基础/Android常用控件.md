...
> 注:Android 8.0(API 26)起发送通知必须先创建 NotificationChannel 并传给 Builder,否则通知不会显示:

```java
// Android 8.0+ 必须指定 channel
...
不能在 Activity 的 onCreate 方法中直接创建展示,要在生命周期完成后才能展示:
...
PopupWindow 不能跟随软键盘位置移动,会覆盖,可以使用 Dialog 替代:
...
> 注:`display.getWidth()/getHeight()` 已废弃,应改用 `display.getSize(Point)` 获取尺寸。

禁用取消的两种方式:

- 方法一:`setCanceledOnTouchOutside(false)`:按对话框以外的地方不起作用,按返回键还起作用
- 方法二:`setCancelable(false)`:按对话框以外的地方不起作用,按返回键也不起作用

> 设置窗口参数要在 show() 方法调用后才有效:
...
在 Activity 中弹出软键盘,在 AndroidManifest 中的相应 activity 下添加:
...
获取 window,通过获取 Activity 相关的 context 获取:
...
webSettings.setJavaScriptEnabled(true);          // 支持 js 脚本
...
> 注:以下旧写法已废弃,整理时已替换或移除:
> - `setPluginsEnabled(true)` 在 API 21+ 无效,改用 `setPluginState(WebSettings.PluginState)`;
...
> 注:Android 11(API 30)起,自定义 View 的 Toast 以及后台线程弹 Toast 受到限制,建议改用 Snackbar:

```java
Snackbar.make(view, "提示内容", Snackbar.LENGTH_SHORT).show();
```
