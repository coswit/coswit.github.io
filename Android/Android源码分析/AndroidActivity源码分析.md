...
App 进程侧的核心类:ActivityThread 持有 ApplicationThread(Binderstub,接收 AMS 回调)与 H(主线程 Handler),LoadedApk 负责创建 Application:
...
```
## 3. Activity 的启动:performLaunchActivity
...
## 4. ActivityManager 与 ActivityManagerService

### ActivityManager.getService:AMS 的获取

AMS 的获取通过 ActivityManager 的 IActivityManager.aidl 完成:
```java
