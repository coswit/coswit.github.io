...
| Looper.cpp | `frameworks/base/libs/utils/`（Native Looper） |
| Looper.h | `frameworks/base/include/utils/` |
...
Android 系统中有一个 Native 的 **servicemanager** 守护进程（小写指进程本身，以与 Java 层的 ServiceManager 类区分），统筹管理所有 Service；成为 Service 的必要条件是在 servicemanager 中注册。
...
另外注意数量关系：Java 层**每一个** Binder 对象都拥有自己的 JavaBBinderHolder，并经它惰性创建**一个独立的** JavaBBinder——不同的 Binder 对象对应不同的 JavaBBinder，而非全局共用一个。

### 1.2.4 AMS 响应请求：JavaBBinder 的请求转发

Native 层的 Binder 架构中，代码里虽然调用的是 Binder 类提供的接口，但对象本身就是实际的服务端业务对象——例如 MediaPlayerService 对象、AudioFlinger 对象，业务与 Binder 服务器端合为一体；而 Java 层的 JavaBBinder 却**与业务完全无关**。它如何实现不同业务？看它的 onTransact：
...
> 勘误：原书在本章把 BpBinder 误写为 BpProxy、把 JavaBBinder 的基类 BBinder 误写为 BnBinder（AOSP 中并不存在这两个类名），本笔记一律按 AOSP 实际类名标注。
...
由此还可以理解**匿名 Binder**：Binder 实体并非只能通过 ServiceManager 发布——任何 Parcel 都能携带它。典型例子是 `registerReceiver`：客户端把 `InnerReceiver`（LoadedApk.ReceiverDispatcher 的内部类）通过 `writeStrongBinder` 直接递给 AMS，这个 Binder 服务从未在 servicemanager 注册过，只有 AMS 拿得到它的引用。
...
Binder 解决的是跨进程调用的问题，而同一线程内的工作调度要靠消息循环——本章第二个主角 MessageQueue 由此登场。Android 2.3 以前，只有 Java 世界的居民有资格向 MessageQueue 添加消息；从 2.3 开始，**MessageQueue 的核心部分下移至 Native 层**，Native 世界也能利用消息循环处理自己世界的事情。从此 MessageQueue 心系 Native 和 Java 两个世界。
...
### 1.3.2 提取消息：next 与 enqueueMessage
...
final Message next() {
    ...
    for (;;) {
        ...
        synchronized (this) {
            ...
            final Message msg = mMessages;   // mMessages 是消息链表头
            if (msg != null) {
            ...
            // 原书特别提醒记住这段处置逻辑——后文分析 Activity 启动时会用到
...
> 版本说明：上面是 4.0 的 next()，没有同步屏障——`target == null` 的消息在 4.0 中只用于退出队列。屏障（target 为 null 的消息挡住同步消息、只放行异步消息）是 4.1 随 Choreographer 引入 postSyncBarrier 才出现的，见本章 1.4 的对比表。
...
```java
// MessageQueue.java :: enqueueMessage（4.0 节选）
final boolean enqueueMessage(Message msg, long when) {
    // 主线程不允许退出：只有非主线程的 Looper 才能收到退出消息
    ...
    synchronized (this) {
        if (mQuiting) {
            return false;           // 队列已在退出流程，拒绝新消息
        } else if (msg.target == null) {
            mQuiting = true;        // target 为 null：这是退出队列的哨兵消息
        }
        msg.when = when;
        Message p = mMessages;
...
队列的退出由此闭环：`Looper.quit()` 只做一件事——取一个 Message（target 天生为 null）并把它塞进队列：

```java
// Looper.java :: quit（4.0）
public void quit() {
    Message msg = Message.obtain();
    // 直接入队，该消息的 target 保持为 null——这就是退出消息的标记
    mQueue.enqueueMessage(msg, 0);
}
```

enqueueMessage 检测到 target == null 就置位 mQuiting（此后新消息一律被拒），并把这条 when 为 0 的消息插到队头。next() 照常把它当作普通消息取出返回给 Looper.loop()，**loop() 见 target 为 null 即认定是退出消息，直接 return**：

```java
// Looper.java :: loop（4.0 节选）
while (true) {
    Message msg = queue.next(); // might block
    if (msg != null) {
        if (msg.target == null) {
            // target 为 null 是退出消息的特殊标记
            return;
        }
        ......
    }
...
也就是说，4.0 的 next() 里并没有 mQuiting 判断，退出的判定发生在 Looper.loop() 一侧；mQuiting 的作用是在退出消息入队后拒绝后续消息。
...
| 同步屏障 API | 无，4.1 才随 Choreographer 引入 postSyncBarrier | 仍 @hide，但 Handler.createAsync + ViewRootImpl 内部用法延续 | Choreographer 的 doFrame 抢占机制至今没变，面试高频 |
