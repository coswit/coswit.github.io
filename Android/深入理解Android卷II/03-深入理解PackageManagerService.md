...
| `mPermissions`（值为 PermissionEntry，内含 gids） / `mSystemPermissions` | 权限定义表 / 特定 UID 被赋的权限 |
...
readPermissions 解析 `/system/etc/permissions` 目录下的 XML（真机上该目录里通常是每种硬件特性一个文件，如 android.hardware.camera.xml、android.hardware.wifi.xml、android.hardware.sensor.accelerometer.xml，外加汇总的 handheld-core-hardware.xml 与权限映射 platform.xml；**最后处理优先级最高的 platform.xml**）：
...
private PackageParser.Package scanPackageLI(File scanFile,
    ...
    /* 此处略去一大段 Package 升级处理：刚解析得到的是"新"信息，
...
framework-res.apk 还包含几个常用 Activity：**ResolverActivity**（多个 Activity 匹配同一 Intent 时弹出选择框，framework-res 中与之关联的 ChooserActivity 同属这一族）、RingtonePickerActivity（铃声选择）、ShutdownActivity（关机对话框）。它与系统息息相关，故得到 PKMS 的特别青睐：`mPlatformPackage` 保存该 Package、`mAndroidApplication` 保存其 ApplicationInfo、`mResolveActivity`/`mResolveInfo` 单独保存 **ResolverActivity** 的信息——**ResolverActivity 使用频率高，单独保存是为了提高运行时的效率**。从 PKMS 查询满足某 Intent 的 Activity 时，返回的就是 ResolveInfo。
...
> 补充 native 库的第三路分支：若 lib 目录不在应用数据目录下（典型是安装在 SD 卡的应用，其文件实际在 /mnt/asec 下），PKMS 会调用 mInstaller 的 **linklib** 命令，由 installd 把 /data/data/\<pkg\>/lib 建为指向 /mnt/asec 对应目录的符号链接——3.6.1 命令表中 linklib/unlinklib 两条命令的出处正在这里。
...
扫描是耗时耗内存的操作：手机装的程序越多，PKMS 工作量越大，启动越慢。原书作者当时提出的两个优化设想是——**延时扫描不重要的 APK**、**保存 Package 信息到文件并在启动时从文件恢复**（减少读取 APK、解析 XML 的工作量），但受限于 APK 之间比较微妙的依赖关系，二者在当时都未能实现。作者还留下另一个疑问：多核 CPU 架构下，PKMS 能否启动多个线程并行扫描不同的目录——4.0 代码中尚无相关的蛛丝马迹。这些设想与疑问后来都成了现实（见 3.7 节）。
...
注意这里的 **PackageInfoLite** 与 3.3.2 提到的 PackageLite 的区别：二者内容几乎一样，但 PackageInfoLite 实现了 Parcelable 接口，可以经 Binder 在进程间传递——DCS 的分析结果要跨进程交回 PKMS，用的必须是 Info 版本。
...
- 外部存储是否为模拟存储（Environment.isExternalStorageEmulated，模拟存储实际占用内部空间，影响 fitsOnSd 的判定）
...
注意 `mActivities.queryIntent(intent, resolvedType, flags)` 的 flags 是 int，而下面 IntentResolver.queryIntent 的同名参数却是 boolean——中间隔着 ActivityIntentResolver 这一层：

```java
// PKMS.ActivityIntentResolver :: queryIntent（节选）
public List<ResolveInfo> queryIntent(Intent intent, String resolvedType,
        int flags) {
    mFlags = flags;    // 把 flags 先存起来，buildResolveList 排序时要用
    return super.queryIntent(intent, resolvedType,
            (flags & PackageManager.MATCH_DEFAULT_ONLY) != 0);   // int → boolean
}
```

即 ActivityIntentResolver 把 flags 暂存到自己的 mFlags，只把"是否只匹配带 CATEGORY_DEFAULT 的 Activity"这一布尔位传给父类。核心匹配在 IntentResolver.queryIntent：
...
查询 API 的 flags 参数当时有 3 个可选值：**MATCH_DEFAULT_ONLY**（只匹配声明了 CATEGORY_DEFAULT 的 Activity）、**GET_INTENT_FILTERS**（返回的 ResolveInfo 附带 filter）、**GET_RESOLVED_FILTER**（只取最佳结果的 filter）。
...
**让人大跌眼镜的是，dex 优化工作竟由 installd 再委派给 dexopt 进程实现**（由 dalvik/dexopt/OptMain.cpp 定义）。优化产物一般位于 `/data/dalvik-cache/` 目录，文件名形如 `data@app@xxx.apk@classes.dex`；系统库的 cache 同理，如 `system@framework@framework.jar@classes.dex`、`system@framework@core.jar@classes.dex`——把校验（verify）与部分优化提前做掉，每次启动 App 不必重复，代价是首次安装变慢、占用额外空间。

#### 2. movefiles 命令

PKMS 扫描完系统 Package 后发送该命令，movefiles 打开 `/system/etc/updatecmds/` 目录并解析其中的指令文件（真机上的文件是 google_generic_update.txt，内容形如）：
...
指令文件以 `#` 开头的行为注释，之后每条指令由"目标包名：源包名"与紧随其后的缩进目录名组成——即把源包名下这些数据目录合并进目标包。**该命令与系统升级有关**（厂商借它在升级时搬移应用数据）。

#### 3. freecache 命令

DeviceStorageMonitorService（磁盘空间监控服务，原书第 3 章 SystemServer 分析中介绍过）发现空间不足时，经 PKMS 的 freeStorageAndNotify 通知 installd 清理：
...
| 开机扫描 | 单线程串行扫全部目录 | 并行扫描 + 缓存优化 | 8.0 起 PKMS 扫描按目录并行化，并用 packages-cache 等手段缩短解析时间——原书 4.3.2 节末的并行扫描疑问与"从文件恢复信息"的设想后来都得到解答 |
