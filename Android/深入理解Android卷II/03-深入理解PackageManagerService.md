...
// (1) 调用 PKMS 的 main 函数;第二个参数判断是否为工厂测试,此处不考虑
...
try {
    ...
    firstBoot = pm.isFirstBoot();             // (2)
} ......
try {
    pm.performBootDexOpt();                   // (3) 对已安装应用做 dex 优化
} ......
try {
    pm.systemReady();                         // (4) 通知系统进入就绪状态
} ......
```
...
readPermissions();                    // (1) 从 /system/etc/permissions 读权限
mRestoredSettings = mSettings.readLPw(); // (2) 读上次的"账本"
```
...
synchronized (mPackages) {
    /* 此段代码约 300 行,主要做四方面工作:
       (1) 如果该 Package 声明了 uses-library,判断该 library 是否在 mSharedLibraries 中
       (2) 如果声明了 sharedUser,处理 SharedUserSettings(由 Settings 的 getSharedUserLPw 完成)
       (3) 处理 pkgSetting(由 Settings 的 getPackageLPw 完成,含 UID 分配/沿用)
       (4) 调用 verifySignaturesLP 检查该 Package 的签名 */
}
...
扫描是耗时耗内存的操作：手机装的程序越多，PKMS 工作量越大，启动越慢。原书作者当时设想的优化方向有二：**延时扫描不重要的 APK**、**把 Package 信息保存到文件并在下次启动时直接恢复**（省去重新读取 APK、解析 XML 的开销）。前者受限于 APK 之间微妙的依赖关系——到底延时扫描哪些 APK 难以确定；作者同时疑惑，多核 CPU 架构下 PKMS 完全可以启动多个线程扫描不同目录，但 4.0 代码中尚无相关蛛丝马迹。这两类优化后来都成了现实（见 3.7 节）。
...
int install_app(transport_type transport, char* serial, int argc, char** argv)
{
    ...
    ......  // (1) 4.0 新增安装包 Verification 功能,见 3.4.6
    pm_command(transport, serial, argc, argv);   // (2) 执行 pm 命令
cleanup_apk:
    // (3) 在手机中执行 shell rm 删除刚才传送的 APK。
...
final boolean startCopy() {
    ...
    try {
        ...
        if (++mRetries > MAX_RETRIES) {
            ...
            handleStartCopy();   // (1) 调用派生类的 handleStartCopy
            res = true;
        }
    } ......
    handleReturnCode();          // (2) 调用派生类的 handleReturnCode,返回处理结果
    return res;
}
```
...
public void handleStartCopy() throws RemoteException {
    ...
    if (onInt && onSd) {
        ...
        try {
            ...
            // (1) 调用 DCS 的 getMinimalPackageInfo,得到一个 PackageLite 对象
        ...
        if (loc == PackageHelper.RECOMMEND_FAILED_INVALID_LOCATION) {
            ...
            // (2) 根据推荐安装路径,调用 installLocationPolicy 再检查
    ...
    // (3) 根据安装位置创建 InstallArgs:内部存储为 FileInstallArgs,否则 SdInstallArgs
    ...
    if (ret == PackageManager.INSTALL_SUCCEEDED) {
        ...
        if (requiredUid != -1 && isVerificationEnabled()) {
            ......  // (4) Verification 流程,见 3.4.6
        } else {
            ret = args.copyApk(mContainerService, true);   // (5) 调用 args 的 copyApk
        }
    }
    mRet = ret;   // 确定返回值
}
...
注意区分两个"轻量级"结构：`PackageParser.parsePackageLite` 返回的是 **PackageLite**；而 getMinimalPackageInfo 返回给 PKMS 的 **PackageInfoLite** 成员变量与之类似，但它实现了 Parcelable 接口，可通过 Binder 在进程间传递——这正是它被选作跨进程返回值的原因。
    ...
    mHandler.post(new Runnable() {
        public void run() {
            ...
            if (res.returnCode == PackageManager.INSTALL_SUCCEEDED) {
                args.doPreInstall(res.returnCode);          // (1) FileInstallArgs 的 doPreInstall
                synchronized (mInstallLock) {
                    installPackageLI(args, true, res);       // (2) 关键:真正的安装
                }
                args.doPostInstall(res.returnCode);          // (3) FileInstallArgs 的 doPostInstall
            }
            ...
            if (!doRestore) {
                // (4) 抛一个 POST_INSTALL 消息给 mHandler 处理
...
flags 参数目前有 3 个可选值：`MATCH_DEFAULT_ONLY`、`GET_INTENT_FILTERS`、`GET_RESOLVED_FILTER`。
...
4.0 的 UserManager 只是雏形，但"每个 User 安装自己的应用"的设想在 4.2 的多用户（平板分身）中变成了现实，`getUid(userId, uid)` 的位组合方式也一直沿用。原书还提示，其思路与小米手机的访客模式颇为相似。
...
原书"本章学习指导"还列出几块因篇幅所限未展开、建议自行研究的内容：APK 安装到 SD 卡以及从内部存储搬到 SD 卡的流程（`SdInstallArgs`/MountService，原书放在卷 III）；签名管理与 dex 优化的细节；权限管理（推荐延伸阅读 *Application Security for the Android Platform* 一书）。学习 PKMS 时宜跳出数据结构的具体形态，从目的与功能角度把握几条主线：构造函数的启动扫描、APK 安装/卸载、Intent 匹配查询。
