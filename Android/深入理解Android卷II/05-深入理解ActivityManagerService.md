...
5. **第五条线**：以一个 Crash 的应用进程为出发点，分析 AMS 如何处理该应用进程死亡后的清理与重启（5.7 节）

此外还统一分析与 AMS 应用进程调度、内存管理相关的知识（5.6 节）。ContentProvider 与 AMS 的交互本章只涉及必要的部分，它自身另有专门的一篇笔记分析。
...
| `mProviderMap` 系列 | ContentProvider 登记表 |
...
public static final Context main(int factoryTest) {
    ...
    m.mBatteryStatsService.publish(context);      // BSS，耗电统计服务
...
main 函数里有一处 wait 与一处 notifyAll：main 函数先等 AThread 创建 AMS 对象，AThread 再等 main 完成后续工作——这种双线程互等在 Android 代码中比较少见。startRunning 传入的四个参数全为 null：它本应设置 mTopComponent/mTopAction/mTopData（系统"最顶层组件"的候选信息），null 时仅保存默认值；由于此时 mSystemReady 还是 false，函数做不了太多事就直接返回了——真正的工作由稍后的 systemReady 接手。
...
> 原书在此有一条定制系统的关键警告：**千万不要去掉 /system/app/SettingsProvider.apk**，否则系统将无法正常启动。
...
```java
// systemReady 第二阶段（节选）
// 杀掉那些不该在 AMS 未就绪时就跑起来的进程——
// AMS 未就绪时，只有声明了 persistent 的进程才被放行启动（isAllowedWhileBooting
// 只豁免 persistent），其余启动请求一律进 mProcessesOnHold 挂起；
// 这里清理的是 persistent 之外不知何故已经跑起来的进程
synchronized(this) {
    if (procsToKill != null) {
...
> 同样的警告也适用于 SystemUi：第三阶段 goingCallback 里启动的 SystemUi（状态栏所在进程）是精简 ROM 时**不能删除**的组件之一。

**ACTION_BOOT_COMPLETED 在哪发？** Home 启动后，`activityIdleInternal` 被调用（见 5.3 节），其中检查 `mBooting` 并调用 `finishBooting`：设置系统属性 `sys.boot_completed=1`，启动 `mProcessesOnHold` 中等待的进程，注册一个 `ACTION_QUERY_PACKAGE_RESTART` 广播接收者，随后才发送 `ACTION_BOOT_COMPLETED` 广播（要求 `RECEIVE_BOOT_COMPLETED` 权限）。此外还会调度一条每 15 分钟触发一次的检查消息（CHECK_EXCESSIVE_WAKE_LOCKS_MSG）：定期检查各应用进程的 WakeLock 使用情况，持有时间过长的进程将被 AMS 关闭。
...
startActivityAndWait 参数众多，关键的几个：`caller`（调用进程的 IApplicationThread，本例为 null）、`resultTo`（接收 startActivityForResult 结果的 Activity）、`grantedUriPermissions`（与 URI 授权有关，5.3.3 的授权小节会看到它如何被消费）。函数体直接转调：
...
**第二阶段**——调用核心函数 `startActivityLocked(caller, intent, ...)`（5.3.3 节）。**第三阶段**——处理返回值：am 设置了 -W，故 `START_SUCCESS` 时把 outResult 加入 `mWaitingActivityLaunched` 并 `mService.wait()` 阻塞。唤醒路径要等到 Activity 真正显示出来：其 ActivityRecord 的 `windowsVisible` 被调用（WMS 汇报窗口可见），内部触发 `reportActivityLaunchedLocked`，它以 notifyAll 唤醒所有等待者，wait 才返回。
...
if (mMainStack) {
    // app switch 检查：仅当当前前台 Activity 不属于调用者（uid 不同）时才检查——
    // 同一应用内部切换不受制约。若 AMS 当前禁止切换，只能把请求存入
...
授权处理也是这段的收尾工作：若 Intent 携带 URI 授权信息，AMS 会调用 `grantUriPermissionLocked`/`grantUriPermissionFromIntentLocked`，把对应 URI 的读写权限记入目标应用的 UriPermission 结构。原书作者在此有一段实战体会：解决 DRM 加密图片的浏览问题时，由于 startActivity 等函数**不能**设置授权，最终只能修改 Gallery3D 工程并为其添加 use-permission 项。

最后的 `startActivityLocked(r, newTask, ...)` 把 ActivityRecord 添加到 mHistory 末尾，并调用 **`resumeTopActivityLocked(null)`**。

#### 1. resumeTopActivityLocked 与 startSpecificActivityLocked
...
**之一：进程登记**。按 pid 找到 ProcessRecord（AMS 在 fork 前已创建；找不到说明是 AMS 不知道的进程，直接 kill）。注意代码里还有一个分支：若该 ProcessRecord 的 `app.thread != null`，要先调 `handleAppDiedLocked` 清理——**这是多线程环境下的典型竞态**：进程死亡后、binderDied 回调尚未执行前，用户恰好又启动了同一进程，就会在这里撞上"Record 还挂着旧 thread"的情况。随后注册 `AppDeathRecipient`（进程死亡通知-death notification-的接收对象，`binderDied` 回调见 5.7 节）；设置 thread、调度优先级、oom_adj 等成员；撤销 PROC_START_TIMEOUT_MSG——**进程启动成功**。
...
#### 3. realStartActivityLocked 与 scheduleLaunchActivity
...
#### 4. startPausingLocked：先暂停当前 Activity
...
sequenceDiagram
    ...
    AMS->>AMS: startActivityMayWait解析Intent-PKMS查询
...
发送端还有一个值得注意的标志 `FLAG_RECEIVER_REPLACE_PENDING`：设置后，若队列中已存在与本次 Intent 完全相同的未处理广播，新广播会替换旧的（而不是再排一条）。但原书特别提醒：**千万不要以为设置了该标志就一定能省掉多余的广播发送**——旧广播可能已开始处理，此时替换并不发生。
...
梳理完派发流程，再补两点队列设计，以及常与广播一起考察的 ANR 超时全景。
...
Service 的处理流程与 Activity 类似但更简单，原书以两张流程图"授人以渔"，这里转成文字版。服务端链路：

- 客户端 `ContextImpl.startService` → AMS 的 `startService` → `startServiceLocked`，先经 `retrieveServiceLocked` 查询或创建 **ServiceRecord**（Service 在 AMS 侧的档案，含启动项、绑定关系、前台标志）
- 随后 `bringUpServiceLocked` 分两支：**进程已存在**直接 `realStartServiceLocked`；**进程不存在**走 `startProcessLocked` 拉起进程，ServiceRecord 挂入 `mPendingServices`，等 attachApplication 第三阶段再 realStartServiceLocked（与 Activity 冷启动共用该逻辑）
- `realStartServiceLocked` 通知客户端 `scheduleCreateService`（发 `H.CREATE_SERVICE` 消息，主线程 handleCreateService 反射创建 Service、调用 onCreate），然后 `sendServiceArgsLocked` 逐条投递启动项——客户端收到 `H.SERVICE_ARGS` 消息，`handleServiceArgs` 中调用 **onStartCommand**。这条"请求投递"链正是 startService 区别于 bindService 的核心：每次 startService 都会向已创建的 Service 追加一次 onStartCommand
- bindService 则走 `requestServiceBindingLocked` → `scheduleBindService`（发 `H.BIND_SERVICE` 消息，主线程 handleBindService 调用 onBind，再经 publishService 把 IBinder 交给客户端的 ServiceConnection）
...
sequenceDiagram
    ...
    C->>A: startService
    A->>A: retrieveServiceLocked查建ServiceRecord
    A->>A: bringUpServiceLocked分两支
    alt 进程不存在
        A->>A: startProcessLocked-mPendingServices挂起
        A->>S: attachApplication后realStartServiceLocked
    else 进程已存在
        A->>S: realStartServiceLocked
    end
    A->>S: scheduleCreateService发H.CREATE_SERVICE
    S->>S: handleCreateService反射创建-onCreate
    A->>S: sendServiceArgsLocked发H.SERVICE_ARGS
    S->>S: handleServiceArgs-onStartCommand
    S-->>A: serviceDoneExecuting回执
```

客户端三条消息（CREATE_SERVICE/BIND_SERVICE/SERVICE_ARGS）处理完毕后，都会调用 AMS 的 **serviceDoneExecuting** 回执。AMS 侧的 `serviceDoneExecutingLocked` 把该 Service 从 `app.executingServices` 中移出，若该进程已无正在执行的 Service 则撤销 `SERVICE_TIMEOUT_MSG`（生命周期回调的 ANR 计时由此解除），最后 `updateOomAdjLocked`。这是流程图中容易漏掉的一环。bindService 还涉及 ConnectionRecord 与引用计数（bindService 的客户端死亡也要经 cleanUpApplicationRecordLocked 收尾）。

## 5.6 AMS 中的进程管理

前几节分析了 AMS 对四大组件的调度，本节统一看它对进程本身的管理——AMS 对进程的管理仅两个方面：**调节进程的调度优先级和调度策略**、**调节进程的 OOM 值**。

### 5.6.1 Linux 进程管理介绍

- **调度优先级**：Linux 用 nice 值描述（可取 -20~19，值越大越"友好"、被调度几率越低；原书此处误写作 nicer），`setpriority(int which, int who, int prio)` 设置；AMS 用 `Process.setThreadPriority`（内核不区分线程和进程，二者对应同一个数据结构 Task）
...
static final int SERVICE_B_ADJ = 8;            // B List 中的 Service（启动后较久无人使用）
...
static final int CORE_SERVER_ADJ = -12;        // 核心服务所在进程（与 persistent 同级）
...
- **heavy weight process 是什么**：Application 可在 Manifest 中声明 `cantSaveState` 属性，声明了该属性的进程即 heavy weight process——系统无法保证其状态可保存恢复，所以给它专门的 adj 档位，切走时会被特殊对待。可惜 4.0 系统并不支持该属性（PackageParser 不解析它）
- 原书提醒：ProcessList 中大部分变量在 2.3 时代定义于 ActivityManagerService.java，移植到 ProcessList 时**注释并未随系统升级而更新**，对照注释读代码时要留个心眼
...
// （客户端会把 provider 进程"拖"上来，provider 享受客户端的待遇）、Home、previous
// process、heavy weight、backup 等各有分支，最后封顶 maxAdj
...
**这就是"后台进程容易被杀、前台放音乐不容易被杀"的机制本质**——各类保活技巧（空 Service、双进程互拉）本质上都是在人为抬高自己在这张 adj 表中的位置。updateOomAdjLocked 的调用点遍布 AMS（Activity resume、Service 启动、Provider 发布、进程死亡……），检查点非常密集。原书学习指导还有一句忠告：不要试图通过修改这部分代码来优化 Android 的运行效率——进程调度规则向来复杂，只有在大量实验的基础上才能得到一个合适的模型。

### 5.6.5 应用退出 ≠ 进程退出

adj 与 LRU 都调整完之后，还有一个常见疑问要澄清：进程什么时候真正死掉？按返回键退出 Activity 后进程成为 Empty 进程：仍留在 mLruProcesses 缓存里（adj=HIDDEN），下次启动同包组件免 fork 免重建运行环境，冷启动显著变快。真正的退出路径只有三条：被 LMK 杀 / 用户 force-stop / 开发者选项清理。**"杀不干净"是缓存设计的代价而非缺陷**——4.0 时代各种"一键清理"应用热卖，正是用户对这一设计的误解加厂商的顺水推舟。
...
应用进程退出后，之前 AMS 为它注册的死亡通知接收对象-AppDeathRecipient-被回调：
...
`handleAppDiedLocked` → `cleanUpApplicationRecordLocked` 完成清理工作，依次处理：
...
应用进程的诞生很麻烦，善后同样费事：crash 上报（dropbox + 对话框）→ 进程自杀 → binderDied 死亡通知回调 → 清理 Service/Provider/Receiver → 视 persistent 与等待情况决定是否重启。persistent app 与"有 Provider 等待"的进程自动重启；普通 Bad Process 则被记入 mBadProcesses，后台不再自动拉起（用户主动启动时移出记录，重新给机会）。
