...
ContentProvider（以下简称 CP）是四大组件中负责**跨应用数据共享**的组件：对外提供统一的 CRUD 接口，底层对接 SQLite、文件或内存数据。原书第 7 章挑选了**四条分析路线**：
...
Context 家族的结论是 ContextImpl 才是真正的实现者：Context 的 getContentResolver 最终调用它所代理的 ContextImpl 对象：
...
先交代 `MediaStore.Images.Media.query` 的宿主——MediaStore 家族的结构（原书用整张类图讲它，并提醒"MediaStore 类较为复杂，主要原因是它定义了一些同名类，阅读代码时务必仔细"）：**BaseColumns** 定义 `_ID`/`_COUNT` 两个基础列；**MediaColumns** 在其基础上定义所有媒体表共用的字段（`_data` 文件路径、`_size`、`mime_type` 等）；**ImageColumns** 再定义 Image 独有的字段。Image、Video、Audio、Files 各有内部类，Image 内部又嵌套 **Thumbnails** 内部类（查询图片缩略图），层层嵌套是它难读的原因：

```mermaid
graph TD
    BC[BaseColumns-定义_ID和_COUNT] --> MC[MediaColumns-媒体表共用字段]
    MC --> IC[ImageColumns-Image独有字段]
    MC --> VC[VideoColumns]
    MC --> AC[AudioColumns]
    MS[MediaStore] --> IMG[Images内部类]
    MS --> VID[Video内部类]
    MS --> AUD[Audio内部类]
    MS --> FIL[Files内部类]
    IMG --> IM[Media-提供query和EXTERNAL_CONTENT_URI]
    IMG --> TH[Thumbnails-查询缩略图]
```
...
public final IContentProvider acquireProvider(Context c, String name) {
    ...
    synchronized (mProviderMap) {
        ...
        // ProviderRefCount 构造函数只收一个计数（4.0 真码；jBinder 已作为 Map 的 key）
        if (prc == null) mProviderRefCountMap.put(jBinder, new ProviderRefCount(1));
        else prc.count++;
    }
    return provider;
}
...
目标进程启动后第一件大事是调用 AMS 的 attachApplication，其内部 `attachApplicationLocked` 会通过 PKMS 查询运行在该进程中的 CP 信息（存入 mProvidersByClass），并把它塞进 bindApplication 的参数传给目标进程。客户端侧 `handleBindApplication` 处理时（注意时序：**CP 的安装早于 Application.onCreate 与其他一切组件**）：
...
```mermaid
graph TD
    BD[Binder基类] --> CPN[ContentProviderNative-实现IContentProvider]
    CPN --> TR[Transport-ContentProvider内部类]
    CPN -.同文件定义.-> CPP[ContentProviderProxy-Bp端]
    CP[ContentProvider] --mTransport成员持有--> TR
    CP --> MP[MediaProvider]
```

- **ContentProviderNative 从 Binder 类派生并实现 IContentProvider 接口**（Binder 服务端骨架，Bn 端所在）
- 每个 ContentProvider 实例中都有一个 `mTransport` 成员，类型为 **Transport**——它从 ContentProviderNative 派生，是 CP 的 Binder 服务端代表
...
这份简单只属于 Native 层开发者。Java 层面对的是 Android 在 SQLite API 之上庞大而复杂的封装——**SQLiteDatabase 家族有 61 个成员之多**。核心几位：
...
dbopen 的 JNI 实现——**Java 层 SQLiteDatabase 对象在这里与 Native 层 sqlite3 实例绑定**（注意最后注册自定义函数时传的 UTF16_STORAGE 宏取值为 0，故 6.3.5 中 register_android_functions 实际走的是 else 分支）：
...
原书还展示了在私有 openDatabase 之上的公有入口做的三步平台级设置：按 /data 分区的块大小设置数据库页大小（`setPageSize(sBlockSize)`，sBlockSize 取自 `new StatFs("/data").getBlockSize()`）；把 journal mode 设为 **TRUNCATE**（比默认的 DELETE 模式 IO 开销更小）；并把新数据库以 WeakReference 登记进 mActiveDatabases，供系统 idle 时统一维护这些活跃数据库。
...
MediaProvider 建表时创建了一个触发器（Trigger，在指定表上发生特定事情时数据库要执行的操作）：
...
先明确 query 参数与 SQL 语句的映射关系（原书在 ContentResolver.query 处专门给出）：`SELECT projection 指定的列名(projection 为 null 则用 *) FROM 表名(由目标 CP 根据 uri 确定) WHERE selection(selection 中含 ? 通配符时,具体值由 selectionArgs 提供) ORDER BY sortOrder`。理解了这层映射，下面的层层封装才不会迷路。本节起把 Binder 通信的两端简称客户端/服务端，服务端即目标进程中的 MediaProvider。
...
public Cursor query(Uri url, String[] projection, String selection,
    ...
    try {
        ...
        //   （observer 交给 ContentService 统一管理）
...
两端冒出了一批新类型：客户端的 **BulkCursorToCursorAdaptor** 与 **IBulkCursor**（Bp 端）、服务端的 **CursorToBulkCursorAdaptor**（IBulkCursor 的 Bn 端，从 BulkCursorNative 即 Binder 派生）以及 MediaProvider query 返回的 Cursor（真实类型待查）。**query 的难度不在游戏规则，而在它所做的层层封装**。

服务端构造 CursorToBulkCursorAdaptor 时还有两步细节：若传入的 cursor 不是 **CrossProcessCursor** 类型，会先用 CrossProcessCursorWrapper 包装一层再使用；随后 `createAndRegisterObserverProxyLocked(observer)` 为客户端的 observer 建立代理并登记——数据更新通知（原书第 8 章的 ContentService 一线）由此挂上钩。

### 6.4.2 MediaProvider 的 query：SQLiteCursor 的创建
...
**MediaProvider query 返回的游标，真实类型是 SQLiteCursor**。SQLiteCursor 的构造函数把结果集的列名取出缓存到 mColumns 数组，并通过 `DatabaseUtils.findRowIdColumnIndex` 记下 `_id` 列的索引（mRowIdColumnIndex）——ListView/Adapter 靠 _id 定位行，这一索引随后会原样传回客户端。SQLiteQuery 则从 SQLiteProgram 派生，构造时触发 SQL 预编译：
...
graph TD
    ...
    subgraph S2["跨进程适配"]
        BN[BulkCursorNative-Binder派生] --> C2B[CursorToBulkCursorAdaptor-服务端Bn端]
        IB[IBulkCursor接口] --> BN
        B2C -- mBulkCursor --> IB
    end
...
- 版本细节：CursorWrapperInner 在 4.0.1 中直接继承 CursorWrapper，4.0.3 起中间多一层 CrossProcessCursorWrapper（原书图 7-6 按后者绘制）；图中其余关系各版本一致
...
public CursorWindow(String name, boolean localWindow) {
    ...
    // sCursorWindowSize 为 2MB：取 config_cursorWindowSize 的值 2048（单位 KB），
    // 再乘以 1024 换算成字节——与 6.3.3 软堆上限用的是同一个配置值
...
query 打通了跨进程数据通道，通道两端握着的 Cursor 也就成了必须管理的资源。Cursor 是重型资源：**不仅占用一个文件描述符，还共享了一块 2MB 的内存**，务必显式 close。
...
为什么客户端不先拿文件路径、自己打开文件？两个原因：**安全**（CP 不希望客户端绕过它直接读存储设备上的文件，且直读还需额外声明存储权限）与**可扩展性**（统一接口既可读实际文件，也可读来自网络的数据，使用者无需关心数据从何而来）。原书用 sqlite3 实查 album_art 表佐证了这条链路：专辑缩略图实际存放于 `/mnt/sdcard/Android/data/com.android.providers.media/albumthumbs/<时间戳>` 这样的路径，`_data` 列存的就是它。
...
带着前几节的机制去看这张表，变化的主线就一条：封装层不断重写，IPC 骨架未动。
...
**原书内容的当代价值**：6.2 的"acquireProvider → AMS 拉起目标进程 → publishContentProviders 唤醒等待者"全链路、客户端与 provider 进程的引用计数联动（incProviderCount/removeContentProvider）、6.4 的 CursorWindow 共享内存与惰性装填、6.6 的 Binder 驱动跨进程传递文件描述符，到今天字字有效；变化集中在应用层 API 的封装高度（Room/Paging/SAF）与 SQLite Java 层框架的整体重写。做系统开发或性能排查（列表滑动卡顿查 window 再装填、Cursor 泄漏查 finalize 警告）时，这几节仍是必背。
