...
| EasAuthenticatorService.java | packages/apps/Email（Exchange 账户的 authenticator 即在 Email 应用内） | AAS 实例 |
...
ContentObserver 与 ContentProvider 非常类似：**内部都定义了一个 Transport 类参与 Binder 通信**。
...
**onChange 的 Binder 调用使用了 FLAG_ONEWAY 标志**：只需将请求发给 Binder 驱动即可返回，无须等待客户端处理完成。因此即使客户端 1 在 onChange 中恶意浪费时间，也不会阻塞客户端 2 的 update。
...
**问题三**：分析 Cursor query 流程时曾看到 ContentObserver 的身影（通知机制与 Cursor 的 requery），回过头去分析 query 流程中与 ContentObserver 相关的部分，所涉及的流程比本节内容还要多——原书将其留作读者自行研究。

## 7.3 AccountManagerService 分析

通知机制讲完，转向本章第二个主角。AccountManagerService 负责管理手机中用户的 online 账户。先看它的创建：
...
uid 在这里的用途值得展开：原书作者的测试机上有 3 个 AAS 服务，其中**同一个 uid 对应两个服务**——uid 由 PKMS 解析 APK 时赋予（Settings.java 的 newUserIdLPw 函数），同一个 APK 里的多个服务自然共用同一个 uid，所以 uid 标识"提供该服务的 APK 是谁"。新旧服务列表比对时，组件名相同而 uid 不同，说明该服务已由另一个 APK 提供（APK 被卸载重装或替换），缓存信息需要刷新并通知监听者。
...
下面通过"为 Exchange 账户添加一个用户"的实例（EasAuthenticatorService）分析 AccountManagerService 的工作流程。AccountManagerService（下称 AMSvc）运行在 SystemServer 中，客户端必须借助 **AccountManager** 提供的 API 使用它。
...
public AccountManagerFuture<Bundle> addAccount(
        ...
        AccountManagerCallback<Bundle> callback,  // 这两个参数决定如何取结果：
        Handler handler)                // callback/handler 为空时，客户端须另起线程
                                       // 调 AccountManagerFuture.getResult 取结果
```
...
> 勘误：方法名 `addAcount` 少了一个 c——这不是抄写笔误，而是 AOSP 4.0 源码自身的拼写（后续版本才更名为 addAccount），原书照录了它，本笔记按 4.0 真实方法名保留。
...
**AMSvc 从不渲染 UI，所有账户相关界面都在 authenticator 应用里**；AMSvc 与 AAS 的每一次交互都通过 bindService 建立、用完即断的连接完成。注意 onServiceConnected 的执行者：bindService 的回调由 **SystemServer 进程主线程的 ActivityThread** 驱动——AMSvc 侧 Session.run 实际运行在这个线程上（原书图 8-10 为简化未画出 ActivityThread）。
...
private class Transport extends IAccountAuthenticator.Stub {
    public void addAccount(IAccountAuthenticatorResponse response,
        ...
        checkBinderPermission();   // 检查权限（enforce 的是 signature 级的 ACCOUNT_MANAGER）
        try {
...
sequenceDiagram
    ...
    App->>AMSvc: ② addAcount-mResponse-检查权限
    ...
    alt options带用户名密码
        ...
        AAS-->>AMSvc: ⑧a onResult-携带账户信息
    else 需要用户输入
        AAS-->>AMSvc: ⑧b onResult-携带KEY_INTENT
    end
...
回看 7.3.2 的完整流程，addAccount 一共跨越了客户端、AMSvc、AAS 三方，靠 AmsTask 与 Session 两个回调壳把异步结果接力回客户端。本节从技术上说涉及 Java concurrent 类（FutureTask/ActiveObject 模式）与 Binder 双向回调的复合。AccountManagerService 及相关类的设计非常巧妙，值得重温 RegisteredServicesCache 的结构及 addAccount 的处理流程并认真体会。
...
// 针对 mSyncAlarmIntent 的接收者——这两项都在非工厂测试模式下才注册
if (!mFactoryTest) {
...
- **AuthorityInfo**（账户 + 数据项的同步信息）：一个账户（含 account、type 两个属性）可对应多种数据项（authority），如原书真机上的 `com.google.android.apps.books`（syncable=0）与 `com.android.email.provider`（syncable=1）——同一账户下不同数据项的开关互相独立。其中的 **periodicSync** 控制周期同步的时间（单位秒，默认 86400 即一天）；**syncable** 属性的可选值为 true/false/unknown（代码中对应 1、0、-1）
...
```java
Account emailSyncAccount = new Account("fanping.deng@gmail.com", "com.google");
...
// 总开关：Master Sync 是否打开（对应 accounts.xml 的 listen-for-tickles）
...
层层考验通过后，maybeStartNextSyncLocked 最后调用 **dispatchSyncOperation** 真正派发同步操作（原书此处把调用者误写为 manageSyncAlarmLocked——它只负责设闹钟，并不派发）：
...
本章两个主角的后续走向截然不同：**通知机制几乎原样存活，同步体系被工作调度新范式取代**。逐项对比：
...
**原书内容的当代价值**：7.2 的通知树机制是全卷变化最小的一章——`ObserverNode` 的匹配派发、`selfChange` 语义、FLAG_ONEWAY 防阻塞、DeathRecipient 自清，今天读 AOSP `ContentService.java` 仍是同一套；7.3/7.4 的账户与同步体系则要区分对待：API 面缩小、新代码不再直接使用，但"RegisteredServicesCache 插件化服务壳 + Session 双向 Binder 桥梁 + 多触发源合并调度"的设计模式仍是可迁移的系统设计经验。

> 原书学习指导给出的上手指引：想编写自己的 AuthenticatorService，参考 SDK 中 AbstractAccountAuthenticator 的说明；想编写同步服务，参考 AbstractThreadedSyncAdapter 的文档；addAccount 流程中用到的 Java concurrent 类（FutureTask 等）是必修基础；进阶可研究 SyncStorageEngine 中同步信息的统计。原书另推荐了《Pattern-Oriented Software Architecture，Volume 2》（POSA2）。
