...
graph TD
    ...
    ATMS -->|同进程同锁| WMSC
    AMS -->|Binder 与 LocalServices| ATMS
```
...
graph TD
    ...
    AMS -->|注册| LS["LocalServices 即 ActivityManagerInternal"]
    ATMS -->|注册| LS2["LocalServices 即 ActivityTaskManagerInternal"]
```
...
| 节 | 链路/主题 | 核心类 | 入口调用链 |
|---|---|---|---|
| 8.2 | AMS 诞生 | `ActivityManagerService.Lifecycle` | `SystemServer#startBootstrapServices` → `ActivityTaskManagerService.Lifecycle` → `ActivityManagerService.Lifecycle` → `setSystemProcess` → `systemReady` |
| 8.3 | Activity 启动 | `ActivityStarter`、`ClientTransaction` | `IActivityTaskManager#startActivity` → `ActivityStarter#execute` → `realStartActivityLocked` → 客户端 `TransactionExecutor#execute` |
| 8.4 | 广播派发 | `BroadcastQueueModernImpl` | `broadcastIntentWithFeature` → `BroadcastController` → 每进程队列调度 |
| 8.5 | Service 与 FGS | `ActiveServices` | `startService`/`startForegroundService` → `startServiceLocked` → `bringUpServiceLocked` |
| 8.6 | 进程与 OOM | `OomAdjuster`、`CachedAppOptimizer` | `updateOomAdjLocked` → oom_score_adj 同步 lmkd |
| 8.7 | Crash 与 ANR | `AppErrors`、`AnrHelper` | `handleApplicationCrash` / `appNotResponding` → `AppExitInfoTracker` 记档 |
...
4.0 时代 Home 启动靠 `mMainStack.resumeTopActivityLocked(null)` 兜底，Android 16 源码是显式的 `startHomeOnAllDisplays`——多显示器时代，每个 Display 都要有自己的 Launcher。`ACTION_BOOT_COMPLETED` 仍在 Home Activity 空闲（`activityIdle`）后由 `finishBooting` 发出，机制与 4.0 相同。
...
### 8.3.3 进程不存在时：startProcessAsync 先释放 wm 锁
...
graph LR
    ...
    PC -->|runnable at 时间戳| S
    S --> R["运行槽位:最多 4 个"]
    S --> COLD["冷启动槽位:同时仅 1 个"]
    R --> D[派发后可续发批量广播摊薄 OOM 调整]
```
...
注意 `Build.HW_TIMEOUT_MULTIPLIER`：Android 16 源码的超时值会按硬件档位缩放，裸记"20 秒"要带这个系数意识。
...
### 8.6.2 杀进程机制：内核 LMK 换为 userspace lmkd 与 PSI
...
### 8.7.4 AppExitInfoTracker：进程退出原因记录
