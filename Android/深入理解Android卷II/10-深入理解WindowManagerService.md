...
> 版本注意：WMS 的骨架（Session 握手、addWindow 校验、relayout 协商、按类型分层）多年来保持稳定；但 Android 11 重构了 insets 分发（InsetsState/InsetsSourceControl 成为 relayout 的出入参），Android 12 起引入 BLAST 同步合帧与 shell Transition，relayout 交接给客户端的从包装好的 Surface 变成了 SurfaceControl。原书卷II 的 WMS 一章基于 4.0 时代源码，本章以当前 main 源码为主体，与旧版的差异不再单独成节，只在涉及处行内标注。
...
| addWindow | 10.4 | App 主线程 → system_server android.display | `ViewRootImpl.setView` → `Session.addToDisplayAsUser` → `WMS.addWindow` |
| relayout | 10.5 | App 主线程 ↔ system_server | `performTraversals` → `Session.relayout` → `WMS.relayoutWindow` → `createSurfaceControl` |
| remove | 10.7 | App 窗口线程 → system_server | `ViewRootImpl.doDie` → `Session.remove` → `WindowState.removeIfPossible` |

全局图景一张图——启动在 system_server 侧自上而下完成；之后每个窗口的生命周期都是"客户端发起、WMS 校验落账"：
...
flowchart TD
    ...
    Relayout --> SF[客户端经 BLASTBufferQueue 提交事务到 SurfaceFlinger]
```
...
| android.display（DisplayThread） | WMS 的主战场：`main` 用 `runWithScissors` 把构造切到这个线程执行，`mH` Handler 也绑定在它上面 |
...
| `WindowManagerGlobal` | 进程单例。维护 `mViews`/`mRoots`/`mParams` 三个平行数组：已添加的根 View、各自的 ViewRootImpl、各自的 LayoutParams；进程内唯一的 `IWindowSession` 也由它懒创建并缓存 |
| `ViewRootImpl` | 每个窗口一个。持有 `IWindow.Stub`（`mWindow`，给 WMS 回调用）、遍历的执行者（measure/layout/draw 三板斧）、BLASTBufferQueue |
...
| 类 | 职责 |
|---|---|
| `Session` | 每个与 WMS 交互的进程一个，`IWindowSession.Stub`，同时实现 `IBinder.DeathRecipient` 监听进程死亡。`openSession` 创建后由客户端缓存复用 |
...
**双界对应**是读懂 WMS 的第一把钥匙：服务端只有 `WindowState` + token，客户端才有 View 树和 `ViewRootImpl`——和第 9 章 ServiceRecord 与 Service 的关系一模一样。`addWindow` 时客户端把 `mWindow`（IWindow.Stub）传过去，WMS 以它的 `asBinder()` 为键放进 `mWindowMap`；此后服务端要通知客户端（如窗口 resize、执行 transaction）就沿这条 Binder 反向调用。
...
`mGlobalLock` 就是 ATMS 持有的那把 `WindowManagerGlobalLock`——Android 10 起窗口管理与 Activity 管理共用一把全局锁。AMS/WMS 的代码互相调用时不必担心锁序问题，代价是任意一方持锁都在事实上阻塞整个 system_server 的调度逻辑，这也是 WMS 的 ANR 会拖垮全局的原因。
...
客户端一句 `mWindowManager.addView(decorView, params)`，到服务端落成一个 `WindowState`，中间要过五道关卡：权限、token、display、查重、参数调整。先看时序，再逐段拆：
...
错误码在服务端产生、异常在客户端抛出，这张映射表就是所有 `BadTokenException` 崩溃的出处，10.8 节会按它整理排查清单。
...
if (type >= FIRST_SUB_WINDOW && type <= LAST_SUB_WINDOW) {
    // 子窗口：attrs.token 必须是本 Session 里已存在的父窗口
...
if (token == null) {
    ...
    // targetSdk O MR1 以上加 Toast 必须持 TYPE_TOAST 的 token
...
| `TYPE_TOAST` | targetSdk O MR1 以下可无 token 直接加 | 需限额时 `token.windowType` 必须是 `TYPE_TOAST` |
| 其他系统窗口 | 用 `client.asBinder()` 新建 `WindowToken` | 误用 ActivityRecord 时置空 attrs.token 重建 |

### 10.4.5 WindowState 落账与输入通道

校验全部通过后，进入"只许成功"阶段——原代码在此处有一句注释 `// From now on, no exceptions or errors allowed!`。先建 `WindowState`：
...
之后是一连串登记：`mWindowMap.put(client.asBinder(), win)`、`win.mToken.addWindow(win)`（按 10.2 的 comparator 插入 `mChildren`）、可接收按键则 `updateFocusedWindowLocked` 抢焦点、`computeImeTarget` 重算输入法目标、`assignChildLayers` 分配层级、`InputMonitor.updateInputWindowsLw` 把窗口列表同步给输入系统，最后把 insets 状态、子窗口依附 frame、兼容缩放回填到出参。窗口至此完成登记，但它还没有尺寸也没有 Surface——这要等 relayout。
...
`addWindow` 只完成了"挂号"，窗口要上屏还差两样东西：确定的尺寸和一块 Surface。这两样在 relayout 里协商：客户端把测量结果报上去，WMS 把最终 frame 与 SurfaceControl 交下来。

### 10.5.1 客户端：performTraversals 的三板斧

ViewRootImpl 收到 `requestLayout` 后经 `scheduleTraversals` 向 Choreographer 注册 `CALLBACK_TRAVERSAL`，垂直同步到来时执行 `performTraversals`。这一遍做三件事，顺序固定：
...
客户端 `relayoutWindow` 有个值得注意的优化分支：如果位置尺寸都没变，就走 `relayoutAsync`——不等服务端结果：

```java
// ViewRootImpl#relayoutWindow(节选)
final boolean positionChanged = ...;
final boolean sizeChanged = ...;
// 位置尺寸都没变时只把 attrs 异步推给 WMS，不阻塞遍历等返回
...
`WMS.relayoutWindow` 超过 300 行，主干分四段。第一段，seq 去重与参数变更处理：
...
`createSurfaceControl` 落到 `WindowStateAnimator#createSurfaceLocked`：真正向 BufferQueue/SurfaceFlinger 申请 SurfaceControl 的地方，之后复制一份引用交给客户端出参。
...
### 10.5.3 BLAST：SurfaceControl 交接与客户端合帧

Android 12 前，relayout 交回的 Surface 内部带着 BufferQueue，客户端往里画、SurfaceFlinger 来取；Android 12 起（BLAST，Balanced Legal AC Timing 或直译"客户端合帧"机制）流程反转为：**WMS 只交 SurfaceControl，客户端自行把绘制内容打包成事务直提 SurfaceFlinger**。窗口首帧的交接在 ViewRootImpl 里：
...
switch (type) {
    ...
    case TYPE_PHONE:        return 3;       // 来电、Dialog 等
...
几个刻意的设计值得咀嚼：壁纸层为 1，唯一比应用窗口（2）还低，WMS 会动态把壁纸窗口"贴"到当前壁纸目标的下方；悬浮窗固定 11，恰好压过普通应用与 Toast（7），但低于 IME（13）和状态栏（15）——这就是"悬浮窗盖不住输入法和系统栏"的出处；同一层值在有 `INTERNAL_SYSTEM_WINDOW` 权限时会有两个取值（如 `TYPE_SYSTEM_ALERT` 的 9/12），系统签名进程的同类窗口天然压在应用之上。

### 10.6.3 DisplayArea：按类型区间划地盘

单个 layer 值只表达"差值"，Android 12 起 WMS 在此之上重构了显示区的组织：每个 `DisplayContent` 内由 `DisplayAreaPolicy` 按窗口类型区间划出若干 `DisplayArea` 分区，WindowToken 按类型"对号入座"。默认策略 `DefaultProvider` 对可信 display 构建的 feature 顺序：
...
flowchart TD
    ...
    IMEP --> Leaf3[Tokens 高于 IME 的系统窗口]
```

这样分区的收益是：放大镜、单手模式、刘海避让这些"作用于一段窗口"的效果，各自只需要锚定一个 DisplayArea 的 Surface，而不必逐个窗口处理。10.4 里 `addWindow` 为窗口新建的 `WindowToken`，正式挂载点就在这里——`DisplayContent.getWindowToken` 查不到时由 `DisplayAreaPolicy` 按类型选中对应分区挂入。
...
`removeView` 默认走延迟路径：`MSG_DIE` 投回窗口线程排队，等当前遍历做完再死，期间 View 进入 `mDyingViews` 待尸名单（再次 addView 时会看到 10.4.1 就地 `doDie` 的处理）；`removeViewImmediate` 则当场执行——但若正处于遍历中，`mIsInTraversal` 会把它降级回延迟路径，防止边遍历边拆树。
...
// token 下窗口清空且非持久 token（如 ActivityRecord）则连 token 一起移除
...
| 崩溃信息片段 | 出错的错误码 | 常见根因 |
|---|---|---|
| `token ... is not valid; is your activity running?` | `ADD_BAD_APP_TOKEN` | Activity 已退出后才 addView；拿别的应用的 token 加应用窗口 |
