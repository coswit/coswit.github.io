...
C 语言是**高级语言**,它的语句要先被翻译成机器能懂的指令序列（**机器语言**）,打包成**可执行目标文件（executable object file）**才能运行。GCC 驱动的翻译分四步（原书 Figure 1.3 的编译系统全景）:

![The compilation system（原书 Figure 1.3）](images/compilation-system.png)
...
![Hardware organization of a typical system（原书 Figure 1.4）](images/hardware-organization.png)

图中要点:CPU（PC、寄存器堆、ALU）经**系统总线 → 内存总线 → I/O 桥**连接主存（DRAM）;I/O 总线经**扩展槽**连接 USB 控制器（键盘/鼠标）、图形适配器（显示器）、磁盘控制器——hello 可执行文件就存在磁盘上。
...
即使一个简单的 hello 程序,系统也要在磁盘、主存、处理器之间复制大量信息——这揭示了一个系统矛盾:**制造更快更小的存储设备,比制造更大更慢的设备成本高得多**。

- **高速缓存存储器（cache memory,高速缓存）** 作为 L1/L2 缓存,是**静态随机访问存储器（Static Random-Access Memory,SRAM）** 技术,缓解处理器与主存（DRAM）的速度鸿沟。
- L1 高速缓存容量数万字节,速度几乎和寄存器一样快;L2 数十万到数百万字节,以专用总线与 L1 相连,访问时间约为 L1 的 5 倍。
...
原书 Figure 1.9 的经典金字塔图（每一层都缓存自下一层,左侧标注速度/价格趋势,比上面的 mermaid 更形象）:

![An example of a memory hierarchy（原书 Figure 1.9）](images/memory-hierarchy.png)

## 1.7 操作系统管理硬件

![layered_view_of_a_system](./images/layered_view_of_a_system.png)
...
![figure1.11_abstractions_provided_by_an_operating_system](./images/figure1.11_abstractions_provided_by_an_operating_system.png)
...
**虚拟内存（virtual memory）** 是对**主存和磁盘 I/O 设备**的抽象,让进程以为自己独占整个主存。每个进程看到的内存是一致的,称为**虚拟地址空间（virtual address space）**,布局如下（地址从下往上增大;原书 Figure 1.13）:

![Process virtual address space（原书 Figure 1.13）](images/process-address-space.png)

- 最底部:**程序代码和数据**——代码从所有进程相同的固定地址开始,随后是全局变量;
- **堆（run-time heap）**:紧随数据之后,调用 malloc 时向上动态增长;
- **共享库的内存映射区域**:如 printf 等 libc 代码与数据;
- **用户栈（user stack）**:编译器用来实现函数调用,向下（低地址）增长;
...
>
> Gene Amdahl 对"提升系统某一部分的性能能带来多大效果"做出了简单而深刻的论断……主要思想是:当我们加速系统的某一部分时,对整体性能的影响取决于这部分原本有多重要、又加速了多少。
...
- **线程级并发**:传统**单处理器系统（uniprocessor）** 只有一个 CPU,靠进程间切换来模拟并发;**多处理器系统（multiprocessor）** 在单一操作系统内核控制下有多个 CPU,能真正并行,随**多核（multi-core,单芯片集成多个 CPU 核,核间共享高层缓存与主存接口）** 的普及成为主流。**超线程（hyperthreading,又称同时多线程 simultaneous multi-threading）** 是让单个 CPU 执行多个控制流的技术:**部分硬件有多份复制（程序计数器、寄存器堆）,其余硬件只有一份（如执行浮点运算的单元）**。传统处理器切换线程约需 2 万个时钟周期,而超线程处理器**逐个周期决定执行哪个线程**——当一个线程等待数据装入缓存时,CPU 可以继续执行另一个线程。例如 Intel Core i7 每个核可执行 2 个线程,4 核系统即可并行执行 8 个线程。
