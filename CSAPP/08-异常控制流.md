...
程序计数器序列叫**逻辑控制流（logical control flow）**。进程好像是独占地使用处理器——每个进程的 PC 值序列与其他进程不相交（重叠区域只在内核代码）。

### 8.2.2 并发流

**并发流（concurrent flow）**:一个逻辑流的执行在时间上与另一个流重叠;多个流并发地运行（**并发执行/concurrent execution**）。单核上的并发其实是**多路复用**——一个进程的指令与另一个交错;只有两个流在不同处理器核上同时运行时,才是真正的**并行（parallel）**。

### 8.2.3 私有地址空间

进程的内存映像独立:它不能读写其他进程的内存（**内核内存与共享内存例外**）——"私有"是对它可读写的地址空间的抽象。
...
有时候主循环更适合**同步地**等待信号（而不是异步处理程序）:

- **sigsuspend**:原子地"解除阻塞 + 休眠",一步完成,消除"先 unblock 再 sleep"间隙中信号丢失的竞态:

```c
sigset_t mask, prev;
sigprocmask(SIG_BLOCK, &mask, &prev);   /* 阻塞 SIGCHLD */
while (!pid_done)
    sigsuspend(&prev);                  /* 原子: 恢复掩码并休眠 */
sigprocmask(SIG_SETMASK, &prev, NULL);
```

- **sigwait**:阻塞直到指定信号集合中任一信号**待处理**,把信号"取走"而不执行处理程序——完全同步地消费信号,避免处理程序的并发复杂性（POSIX 保证不与 sigwait 竞争）。
