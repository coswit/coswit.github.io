...
- **虚拟寻址（virtual addressing）**:CPU 生成**虚拟地址（virtual address,VA）**,经 **MMU（Memory Management Unit,内存管理单元）** 的地址翻译转换为**物理地址（physical address,PA）**。
...
VM（Virtual Memory）思想:虚拟页（Virtual Page Number,VPN）是磁盘块,**DRAM 作缓存**:
...
页表项（Page Table Entry,PTE）含**有效位 + 地址**:有效位 1 → 后面是 DRAM 中物理页号（命中）;有效位 0 → 空地址（未分配）或磁盘地址（未缓存）。
...
**页表基址寄存器（Page Table Base Register,PTBR）** 指向当前进程页表;切换进程 = 切换 PTBR（上下文切换的一部分）。
...
GC（Garbage Collection,垃圾收集）自动识别并释放**垃圾（garbage,不再被引用的已分配块）**:**不可达**即垃圾。根节点:寄存器、栈、全局/已初始化数据区中的指针。
