...
- **文件表（file table）**:全局,每项含**文件位置（offset）、状态标志、引用计数、指向 v-node**;
- **v-node 表（v-node table）**:全局,每项含文件元数据（stat 信息、函数指针）。

```mermaid
graph LR
    D1[进程A fd1] --> F1[文件表项1 offset=k]
    D2[进程A fd2 dup复制] --> F1
    D3[进程B fd4 独立open] --> F2[文件表项2 offset=0]
    F1 --> V[v-node 同一文件]
    F2 --> V
```
