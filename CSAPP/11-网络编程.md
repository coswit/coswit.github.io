...
- 最流行的网络是**以太网（Ethernet,LAN）**:每台主机有唯一 **48 位 MAC 地址**,段内广播帧,各主机按目的 MAC 接收（交换机学习 MAC-端口映射）;
...
**Web 客户端（浏览器）+ Web 服务器 + HTTP**:

- **URL**: `http://www.google.com:80/index.html` → 协议、主机、端口、资源路径;
- **MIME 类型**:text/html、image/gif、text/plain... 告诉浏览器如何处理内容。
