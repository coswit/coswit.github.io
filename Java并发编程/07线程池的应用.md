...
2. **长时间运行的任务**：占住线程，拖慢甚至饿死短任务（应该隔离到专用池；书中建议：限时等待后"告警并放弃"或让任务自己管理时间片——`Thread.yield()` 类的机制）；
3. **依赖 ThreadLocal 的任务**：线程复用让 ThreadLocal 的假设失效（扩展的执行策略改变了线程语义——子任务跑在别的线程，上下文丢了）；
4. **任务对响应时间敏感 / 需要特定执行环境**（GUI 线程）。

结论：**提交任务时就把执行策略约束说清楚**（文档化"该任务要求的池类型"）。线程池的工作队列/大小/策略不该是"运维细节"，而是**应用架构的一部分**——每个池该"服务哪些任务、多大、满怎么办"，在代码评审时确定。
...
- W/C 怎么测：Profiler 或工具测量"等待 IO 的时间占比"；
...
**提交任务时的行为**（顺序很重要，是排错的常识）：

1. 池中线程数 < corePoolSize → 新建线程执行（哪怕有空闲线程）；
2. 达到 core → 任务进 workQueue 排队；
3. 队列满 → 继续创建线程直到 maximumPoolSize；
4. 达到 max 且队列满 → **执行饱和策略（handler）**。
...
队列满且线程达 max 时，新提交的任务交给 RejectedExecutionHandler：
...
线程的"出身"由 ThreadFactory 决定——**生产环境必备的自定义项**：命名（排错时 jstack 里能看到是哪个池的线程）、守护性、优先级、UncaughtExceptionHandler：
...
> 【演进】Android 主线程（UI Thread）是同一思想的现代实现：主线程不能阻塞（ANR），后台工作交给 Handler/协程，UI 更新必须 post 回主线程。Jetpack Compose 乃至 Flutter、SwiftUI 都延续了"UI 单线程 + 后台并行"的结构。
