...
- **内存同步**：锁的获取/释放（无竞争时也有指令开销——CAS + 内存屏障；有竞争时排队挂起）、volatile 内存屏障禁止重排优化——可见性的代价是限制了 JIT（Just-In-Time Compiler，即时编译器）的重排优化；synchronized 在无竞争时的开销很低（第 1 篇演进说明）；
...
- **锁分段（lock striping）**：分解的推广——一组独立的锁对象组成数组，按哈希选锁，访问不同段的线程互不阻塞。ConcurrentHashMap 的经典实现（Java 7 前：默认 16 段，理论上写并发度可达 16）：
...
热点域（hot field）= 所有操作都要访问的共享点（如集中维护的计数器 size）——它把并行操作重新串行化。ConcurrentHashMap 的 size 是**估计值**（各段独立计数、读时汇总/无锁累加），就是主动避免热点域的设计（牺牲精确性换伸缩性）。类似的还有 LongAdder 的分段累加（第 11 篇）。
...
**案例：Map 实现的吞吐量对比**（书中的实验设计，比具体数字重要）——同步 Map（synchronizedMap）在**低竞争**下与 ConcurrentHashMap 差距不大；**高竞争（多写者/多核）**下 synchronizedMap 吞吐断崖式下跌（所有操作串行化在一把锁上），ConcurrentHashMap 只温和下降。另一个反直觉结论：**写密集时读缓存也可以优化**——“重新计算 vs 加锁读取”的权衡里，避免共享热点才是本质。
...
> 【演进】书中表 11.1 的数字成书于 2006 年的单/双核时代，今天核心数翻了两个数量级——Amdahl 的制约更狠了：100+ 核上 F=0.1 意味着 90% 的算力闲置。现代工具链（JFR/JMC、async-profiler、perf）让“测量先行”变得便宜；GC 与 NUMA（Non-Uniform Memory Access，非统一内存访问架构）的跨核缓存一致性成为新的隐形串行点。虚拟线程则把“减少阻塞开销”这条线推向了 OS 调度层。
