...
忘记 try/finally 包住 unlock（或把 try 放在 lock() 成功之前）是显式锁的经典事故：异常路径漏释放 = 永久锁死。
...
- **公平锁**：`new ReentrantLock(true)` 按先进先出 FIFO 授予（默认非公平）。
...
**公平性的代价**：公平锁要挂起/唤醒（排队），吞吐低于非公平锁（非公平允许“插队”：刚释放的锁被正好到来的线程直接拿走，省去上下文切换——ReentrantLock / ReadWriteLock / Semaphore 的默认）。非公平的代价是“可能饥饿”（运气差的线程长期拿不到）。只有需要避免饥饿时才用公平锁（公平锁也不保证严格按 FIFO 唤醒调度）。

**选型**：**优先 synchronized**——不会忘记释放、JVM 优化好（性能差距已被 JDK 6+ 抹平）；需要 tryLock/超时/可中断/非块结构时才用 Lock。混用要小心（同一状态一半由 synchronized 保护、一半由 Lock 保护 = 两把互不相干的锁，等于没保护）。**读代码时把 lock/unlock 当作作用域边界来审**——它的“块”是人脑想象的，编译器不管。
...
- 注意：读锁的让渡策略（已有写线程在排队时，新读者是否还允许插队拿读锁）决定写者是否长期饥饿；竞争激烈且写操作不少时，读写锁可能比独占锁更慢（维护两把锁状态的成本）。
...
> 【演进】AQS 在 JDK 9~14 经历了内部重写（VarHandle 化），API 不变；JDK 21 的虚拟线程有一处著名的交互坑——**虚拟线程 pinned 在 synchronized 块里时不让出载体线程**（JDK 24 才解决），官方因此建议虚拟线程场景的竞争热点用 ReentrantLock（历史上首次出现“场景级建议放弃 synchronized”）。另：自 Java 5 起 wait/notify 的绝大多数用途都应替换为 Condition/并发工具——新代码几乎不该手写条件队列。
