...
## 函数作用域Scope Functions

参考：https://kotlinlang.org/docs/scope-functions.html#function-selection

需要对一个Context Object执行一段代码时，可以通过在对像上使用scope functions传入lambda中，创建一个临时作用域(temporary scope)，无需通过对像名，直接访问对像。作用域函数主要区别在于，Context Object的引用方式是`this`还是`it`。

> execute a block of code within the context of an object。When you call such a function on an object with a lambda expression provided, it forms a temporary scope. In this scope, you can access the object without its name.
...
| **函数**            | **引用上下文对象**     | **返回值**        | **是否为扩展函数**             |
| ------------------- | ---------------------- | ----------------- | ------------------------------ |
| **`let`**           | `it`（可自定义参数名） | Lambda 表达式结果 | 是                             |
| **`run`**           | `this`（可省略）       | Lambda 表达式结果 | 是                             |
| **`run`（非扩展）** | 不适用                 | Lambda 表达式结果 | 否（单独调用，无需上下文对象） |
| **`with`**          | `this`（可省略）       | Lambda 表达式结果 | 否（将对象作为参数传入）       |
| **`apply`**         | `this`（可省略）       | 上下文对象本身    | 是                             |
| **`also`**          | `it`（可自定义参数名） | 上下文对象本身    | 是                             |

### let

在函数体内使用 it 指代对象，返回 Lambda 最后一行，配合安全调用符 `?.` 处理非空判断，或将对象转换为另一种类型：
...
在函数体内直接使用 `this`，返回 Lambda 最后一行。是 `apply`（使用 this）和 `let`（返回计算结果）的结合体。

**典型场景**：既需要配置/调用对象，又需要获取一个最终计算结果。

```kotlin
val result = service.run {
    port = 8080
    queryData() // 返回 queryData() 的执行结果
}
```

### with

非扩展函数，需要把对象作为参数传入 with(obj) { ... }，返回 Lambda 最后一行。对已存在的非空对象进行一组连续的操作，避免重复写对象名。

```kotlin
val user = User("Tom", 18)
val summary = with(user) {
    "Name: $name, Age: $age" // 返回拼接后的字符串
}
```

### apply

在函数体内直接使用 `this` 访问对象属性/方法，返回对象本身。初始化或配置一个对象的属性（配置完成后直接拿到该对象）。
...
在函数体内使用 `it` 指代对象，**返回对象本身**。不改变对象本身，只是做一些副作用操作（如打日志、校验、把数据写入数据库等）。

```kotlin
val numbers = mutableListOf("one", "two", "three")
numbers.add("four")
    .also { println("新元素添加结果: $it") } // 返回 add() 的结果或继续链式调用
```
