...
文件检查：`-e` 存在、`-d` 是目录、`-f` 是文件、`-r/-w/-x` 可读/写/执行。
...
```shell
#!/bin/bash
#调用：./test.sh -ab value -c
while getopts ab:c opt
do
	case "$opt" in
	a) echo "Found -a";;
	b) echo "Found -b, value is $OPTARG";;
	c) echo "Found -c";;
...
每个选项处理完后保存在 variable（上例的 `opt`）中，选项带的参数值保存在 `OPTARG`。
...
#重定向错误到空设备，不输出
2> /dev/null
#标准输出与错误都写入文件（&> 或 > file 2>&1，注意 2>&1 必须在 > 之后）
cmd &> all.log
cmd > all.log 2>&1
#只显示错误，标准输出丢弃
cmd 2>&1 1> /dev/null
