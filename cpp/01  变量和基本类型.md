...
指针(`object->field`)和引用(`object.field)`的区别：

- 指针是一个变量，存储的是另一个变量的内存的地址；引用是变量的别名，与变量共享一地内存地址。
- 指针可以不初始化，指向空值；引用则必须在声明时初始化，且引用所指向的对像在该引用初始化后不能改变。
- 指针可以改变指向，指向不同的对象；引用一旦绑定就不能改变，始终引用同一个对象。
- 指针本身占用内存，引用不额外占用内存。
