...
> 本文是 Java Annotation Processing（注解处理）实战教程，来自 [Mindorks](https://github.com/MindorksOpenSource/annotation-processing-example)：从零实现一个类似 ButterKnife 的 View Binding 库。
>
> Annotation Processing 也是 ButterKnife、Dagger2、Room 等 DI / 代码生成框架的底层基础——它们都在编译期生成胶水代码，从而避免运行时 Reflection 带来的性能与安全问题。

## 导读

- **Part 1: A Practical Approach**：动机——用 Reflection 在运行期扫描注解做 View 绑定（`ViewBinder` 示例），类扫描慢、易抛运行时异常；Annotation Processing 在编译期生成代码，把检查提前到编译期。
- **Part 2: The Project Structure**：四个模块——app（应用）、binder（运行时入口 `Binding.bind()`）、binder-annotations（`@BindView`/`@OnClick`/`@Keep` 三个注解）、binder-compiler（Processor）；注册方式是在 `META-INF/services/javax.annotation.processing.Processor` 中写全限定类名。
- **Part 3: Generate Java Source Code**：在 `process()` 中过滤标注了目标注解的 `TypeElement`（`getEnclosedElements()`/`getAnnotationMirrors()`），再用 JavaPoet（`TypeSpec`/`MethodSpec`，占位符 `$N` 名字、`$T` 类型、`$L` 字面量）生成 `MainActivity$Binding`，输出到 `build/generated/source/apt` 目录。
- **Part 4: Use The Generated Code**：`Binding.bind()` 通过反射按「类名 + `$Binding`」加载生成的类并实例化（ButterKnife 同理，这是全文唯一用到 Reflection 的地方）；最后用 `@Keep` 注解配合 `consumerProguardFiles` 声明 proguard 规则，防止混淆后反射找不到类。

> 应用示例：Dagger 2 如何用 annotation processor 实现 Dependency Injection，见 [设计模式-Dagger2](设计模式-Dagger2.md)。
...
**CompileOnly** allows us to use a library in a module but not make it available when it is distributed. Since all the android applications include this dependency so we are safe to not ship this dependency with our library distribution.
...
- **Messager**: It is used to print messages when the compilation is taking place. We send the error messages that may arise in processing via Messager. Since annotation processor runs in its own separate environment, we can not communicate with the application by any other means.
...
We need to register this Processor to the **javax.annotation.processing.Processor** file using its complete canonical name.
...
- **element.getEnclosedElements()**: Enclosed elements are the elements that are contained in the given element. In our case the element will be MainActivity (TypeElement) and the Enclosed elements will be tvContent, onCreate, bt1Click, bt2Click and other inherited members.
...
- MethodSpec -> addStatement: It will add the code blocks inside a method. In this method we 1st define the placeholders of the statement and then pass the parameters to map those placeholders. Example: addStatement("$N($N)", "bindViews", "activity") (this will generate code bindViews(activity)). PlaceHolders: $N -> names, $T -> type(ClassName), $L -> literals(long etc.).
...
Also, we would want to add the proguard rule in the library itself so that client doesn't have to worry about it.

To do so we need to do two things:

- First: Add proguard rule in the binder library module's proguard-rules.pro file:
...
- Second: Let the gradle know that this proguard rule has to be added in the module that uses it. We will have to add consumerProguardFiles in the build.gradle of the binder library module.
